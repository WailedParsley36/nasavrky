...
const { maxSize, minSize } = (() => {
  ...
  return {
    maxSize: result[0].plocha,
    minSize: result[result.length - 1].plocha,
  };
})();
...
export default function HomePage() {
  ...
  const [pricePerMeterRange, setPricePerMeterRange] = useState({
    min: 0,
  ...
  useEffect(() => {
    setFilteredParcels(
      AllParcels.filter((parcel) => {
        ...
        const matchesSize =
          parcel.plocha >= sizeRange.min && parcel.plocha <= sizeRange.max;

        // Return true only if all conditions are met
        return (
          matchesSearch && matchesPrice && matchesSize && matchesPricePerMeter
        );
      })
    );
    ...
    <>
      {mapEnlarged && (
        <div className="fixed inset-0 z-40 bg-black/80 flex items-center justify-center">
          ...
          <button
            className="fixed top-4 right-4 z-[99999] bg-white text-black p-2 rounded-full shadow"
            ...
            <div className="sticky top-6">
              <div className="rounded-lg mb-4">
                <ParcelMap
                  ...
                  selectedIds={filteredParcels.map((x) => x.id)}
                  onParcelClick={(p) =>
                    router.push(`/parcely/${base64url(p.id)}`)
                  }
                />
                <button
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                ...
                <div className="mb-4 relative">
                  <MultiRangeSlider
                    min={
                      minPricePerMeter == maxPricePerMeter
                        ? 0
                        : minPricePerMeter
                    }
                    max={maxPricePerMeter}
